...
import { IconAmazon } from '../assets/icons/iconAmazon';
import { IconAmx } from '../assets/icons/iconAmx';
          ...
          return (
            <Fragment key={index}>
              {pageData.globalSettings?.footer_com &&
                pageData.globalSettings?.footer_com[0].online_transaction_platform_esa &&
                pageData.globalSettings?.footer_com[0].online_transaction_platform_esa?.includes(
                  supportedPayment.name,
                ) && (
                  <li>
                    <i className="icon [&_svg]:w-full [&_svg]:h-full">{supportedPayment.icon}</i>
                  </li>
                )}
            </Fragment>
          );
